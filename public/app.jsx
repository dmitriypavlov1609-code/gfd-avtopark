  ...
  return (
    <Fragment>
      <div className="page-head">
        <div>
          <h1>Сегодня · 14 мая 2026</h1>
          <div className="sub">► выручка / парк / клиенты в реальном времени</div>
        </div>
        <div className="actions">
          <button>экспорт</button>
          <button className="primary">+ бронирование</button>
        </div>
      </div>

      <div className="stats">
        <div className="stat"><div className="l">► выручка сегодня</div><div className="v">€2 184</div><div className="d"><span className="delta up">+18%</span><span className="lab">vs неделя назад</span></div></div>
        <div className="stat"><div className="l">► активные аренды</div><div className="v">68 / 107</div><div className="d"><span className="delta up">+4</span><span className="lab">vs вчера</span></div></div>
        <div className="stat"><div className="l">► парк · загрузка</div><div className="v">63.5%</div><div className="d"><span className="delta up">+2.1pp</span><span className="lab">7-дн</span></div></div>
        <div className="stat"><div className="l">► NPS · 30 дней</div><div className="v">+72</div><div className="d"><span className="delta up">+5</span><span className="lab">vs прошлый период</span></div></div>
      </div>

      <div className="grid-2">
        <div className="card">
          <div className="h"><div className="t">Сегодняшние бронирования</div><div className="m" style={{cursor:'pointer'}} onClick={()=>setPage('book')}>все →</div></div>
          <div className="b flush">
            <table className="tbl">
              <thead><tr><th>ID</th><th>Клиент</th><th>Машина</th><th>Период</th><th>Сумма</th><th>Статус</th></tr></thead>
              <tbody>
                {BOOKINGS.slice(0,6).map(b=>(
                  <tr key={b.id}>
                    <td><span className="id">{b.id}</span></td>
                    <td><span className="pri">{b.cust}</span><span className="sec">{b.phone}</span></td>
                    <td>{b.car}</td>
                    <td><span className="id">{b.from} → {b.to}</span></td>
                    <td><b style={{color:'var(--cream)'}}>€{b.total}</b></td>
                    <td><span className={'pill ' + b.status}>{({active:'active',pending:'pending',done:'done',cancelled:'cancelled'})[b.status]}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="card">
          <div className="h"><div className="t">Загрузка парка</div><div className="m">{FLEET.length} авто</div></div>
          <div className="b">
            <div className="util-bar">
              <div className="seg rented" style={{width:'62%'}}></div>
              <div className="seg avail" style={{width:'30%'}}></div>
              <div className="seg maint" style={{width:'8%'}}></div>
            </div>
            <div className="util-legend">
              <span>в аренде 67</span>
              <span className="l2">свободно 32</span>
              <span className="l3">сервис 8</span>
            </div>
            <div style={{marginTop:18,paddingTop:18,borderTop:'1px solid var(--line)'}}>
              <div className="util-legend" style={{marginBottom:8}}><span style={{color:'var(--cream-2)'}}>► популярные сегодня</span></div>
              {[
                {n:'BMW Z4 cabrio', v:'5 / 5 · all rented'},
                {n:'Mini Cooper S Cabrio', v:'4 / 5'},
                {n:'VW Touran (family)', v:'3 / 4'},
                {n:'Skoda Fabia (econom)', v:'9 / 12'},
              ].map((r,i)=>(
                <div key={i} style={{display:'flex',justifyContent:'space-between',padding:'7px 0',borderBottom:i<3?'1px dashed var(--line)':'none',fontSize:13}}>
                  <span style={{color:'var(--cream-2)'}}>{r.n}</span>
                  <span style={{fontFamily:"'JetBrains Mono', monospace",fontSize:11.5,color:'var(--cream-3)'}}>{r.v}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="grid-2">
        <div className="card">
          <div className="h"><div className="t">Активные переписки · AI-помощник</div><div className="m" style={{cursor:'pointer'}} onClick={()=>setPage('convo')}>все →</div></div>
          <div className="b flush">
            <table className="tbl">
              <thead><tr><th>Клиент</th><th>Канал</th><th>Последнее сообщение</th><th>Статус</th></tr></thead>
              <tbody>
                {CONVOS.slice(0,5).map(c=>(
                  <tr key={c.id} style={{cursor:'pointer'}} onClick={()=>setPage('convo')}>
                    <td><span className="pri">{c.name}</span><span className="sec">{c.time}</span></td>
                    <td><span className="chan">{c.ch}</span></td>
                    <td style={{maxWidth:280,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{c.last}</td>
                    <td>
                      {c.status === 'ai' && <span className="pill avail">AI</span>}
                      {c.status === 'human' && <span className="pill pending">human</span>}
                      {c.status === 'done' && <span className="pill done">done</span>}
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="card">
          <div className="h"><div className="t">Источники данных</div><div className="m" style={{cursor:'pointer'}} onClick={()=>setPage('sync')}>все →</div></div>
          <div className="b flush">
            {SOURCES.slice(0,5).map((s,i)=>(
              <div key={i} className="sync-row" style={{borderBottom:i<4?'1px solid var(--line)':'none'}}>
                <div className="ico">{s.ic}</div>
                <div>
                  <div className="name">{s.name}</div>
                  <div className="desc">{s.items}</div>
                </div>
                <div></div>
